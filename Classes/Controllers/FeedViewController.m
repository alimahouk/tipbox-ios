...
@interface FeedViewController (PrivateStuff)
...
{
    Intro_InterestsViewController *qsg = [[Intro_InterestsViewController alloc] init];
    qsg.configuration = @"feed";
    qsg.hidesBottomBarWhenPushed = YES;
    self.navigationItem.backBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:@"Feed" style:UIBarButtonItemStylePlain target:nil action:nil] autorelease];
    [self.navigationController pushViewController:qsg animated:YES];
    [qsg release];
    qsg = nil;
}
...
- (void)viewDidLoad
{
	...
	//self.navigationController.navigationBar.topItem.titleView = [appDelegate.tabBarController setupNavBar];
    ...
    NSString *filePath =  [libraryDirectory stringByAppendingPathComponent:@"TBFeedData/feed.txt"];
    
    NSMutableArray *savedFeed = [[NSKeyedUnarchiver unarchiveObjectWithFile:filePath] mutableCopy];
    
    if (savedFeed != nil) {
        feedEntries = [[NSMutableArray alloc] initWithArray:savedFeed];
    } else {
    ...
    dottedDivider.frame = CGRectMake(0, 90, 320, 2);
    dottedDivider.hidden = YES;
    
    qsgIntroLabel = [[UILabel alloc] initWithFrame:CGRectMake(58, 105, 257, 32)];
    qsgIntroLabel.backgroundColor = [UIColor clearColor];
    qsgIntroLabel.textColor = [UIColor colorWithRed:145.0/255.0 green:145.0/255.0 blue:145.0/255.0 alpha:1.0];
    qsgIntroLabel.shadowColor = [UIColor colorWithWhite:1.0 alpha:1.0];
    qsgIntroLabel.numberOfLines = 0;
    qsgIntroLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:SECONDARY_FONT_SIZE];
    qsgIntroLabel.text = @"Pick a few categories that interest you, and we'll show you some tips you'll like here!";
    qsgIntroLabel.userInteractionEnabled = YES;
    qsgIntroLabel.hidden = YES;
    
    qsgIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tipbox_icon_grey.png"]];
    qsgIcon.frame = CGRectMake(13, 104, 32, 32);
    qsgIcon.hidden = YES;
    
    qsgButtonCard = [[UIImageView alloc] initWithImage:[[UIImage imageNamed:@"card_shadow_bg.png"] stretchableImageWithLeftCapWidth:7.0 topCapHeight:7.0]];
    qsgButtonCard.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"bg_paper_texture.png"]];
    qsgButtonCard.opaque = YES;
    qsgButtonCard.userInteractionEnabled = YES;
    qsgButtonCard.frame = CGRectMake(10, 150, 300, 38);
    qsgButtonCard.hidden = YES;
    
    UIButton *qsgButton = [[UIButton buttonWithType:UIButtonTypeCustom] retain];
    qsgButton.adjustsImageWhenHighlighted = NO;
    qsgButton.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"paper_noise.png"]];
    [qsgButton setBackgroundImage:[[UIImage imageNamed:@"feed_button_grey_bg.png"] stretchableImageWithLeftCapWidth:0.0 topCapHeight:0.0] forState:UIControlStateHighlighted];
    [qsgButton addTarget:self action:@selector(showQuickStartGuide) forControlEvents:UIControlEventTouchUpInside];
    qsgButton.frame = CGRectMake(4, 4, 292, 30);
    
    LPLabel *qsgButtonLabel = [[LPLabel alloc] initWithFrame:CGRectMake(4, 4, 292, 30)];
    qsgButtonLabel.backgroundColor = [UIColor clearColor];
    qsgButtonLabel.textColor = [UIColor colorWithRed:157.0/255.0 green:157.0/255.0 blue:157.0/255.0 alpha:1.0];
    qsgButtonLabel.shadowColor = [UIColor colorWithWhite:1.0 alpha:1.0];
    qsgButtonLabel.numberOfLines = 1;
    qsgButtonLabel.minimumFontSize = 8.;
    qsgButtonLabel.adjustsFontSizeToFitWidth = YES;
    qsgButtonLabel.font = [UIFont fontWithName:@"Georgia" size:MIN_MAIN_FONT_SIZE];
    qsgButtonLabel.textAlignment = UITextAlignmentCenter;
    qsgButtonLabel.text = @"Launch Quick Start Guide »";
    
    UISwipeGestureRecognizer *qsgIntroLabelSwipeGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(showQuickStartGuide)];
    qsgIntroLabelSwipeGesture.direction = UISwipeGestureRecognizerDirectionLeft;
    [qsgIntroLabel addGestureRecognizer:qsgIntroLabelSwipeGesture];
    [qsgIntroLabelSwipeGesture release];
    
    UISwipeGestureRecognizer *qsgButtonCardSwipeGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(showQuickStartGuide)];
    qsgButtonCardSwipeGesture.direction = UISwipeGestureRecognizerDirectionLeft;
    [qsgButtonCard addGestureRecognizer:qsgButtonCardSwipeGesture];
    [qsgButtonCardSwipeGesture release];
    ...
    // This view is a special case. Its view loads when the box covers are
    // still closed. We don't want it making pointless timeline calls when there's
    // still no token. ;)
	...
	//[refreshHeaderView refreshLastUpdatedDate]; // Update the last update date.
	
    [timelineFeed.layer addSublayer:dottedDivider];
    [timelineFeed addSubview:qsgIntroLabel];
    [timelineFeed addSubview:qsgIcon];
    [timelineFeed addSubview:qsgButtonCard];
    [qsgButtonCard addSubview:qsgButton];
    [qsgButtonCard addSubview:qsgButtonLabel];
    
    [tbLogo release];
    [qsgButton release];
    [qsgButtonLabel release];
    [super viewDidLoad];
}
...
- (void)timelineDidFinishDownloading
{
    ...
    [global writeValue:[dateFormatter stringFromDate:[NSDate date]] forProperty:@"lastRefreshedDate"];
    timelineDidDownload = YES;
    
    if ([self.feedEntries count] == 0) {
        dottedDivider.hidden = NO;
        qsgIntroLabel.hidden = NO;
        qsgIcon.hidden = NO;
        qsgButtonCard.hidden = NO;
    } else {
        dottedDivider.hidden = YES;
        qsgIntroLabel.hidden = YES;
        qsgIcon.hidden = YES;
    ...
    NSString *filePath =  [folderPath stringByAppendingPathComponent:@"feed.txt"];
    BOOL isDir;
...
{
    // Add an object to the end of the array for the "Load more" cell.
...
- (void)tapTimerFired:(NSTimer *)aTimer
{
    ...
    if (tapTimer != nil) {
        ...
        if (lastTappedRow != tappedRow) { // Collapse any other cell (unless if it's the same one, this will cause fuckage).
            ...
            if (indexPath_lastTappedRow.row != [self.feedEntries count]) { // We don't want it sending wrong messages to the "Load more" cell.
...
- (void)loadMoreFeedEntries
{
    ...
    [self downloadTimeline:@"getuserfeed" batch:++batchNo];
}
...
- (void)dealloc
{
    ...
    [qsgIntroLabel release];
    [qsgIcon release];
    [qsgButtonCard release];
    [super dealloc];
}
