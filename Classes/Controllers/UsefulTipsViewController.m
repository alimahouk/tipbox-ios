...
@implementation UsefulTipsViewController
...
- (void)viewDidLoad {
    ...
    [self downloadTimeline:@"gettipslikedbyuser" batch:0];
...
- (void)loadMoreFeedEntries
{
    ...
    [self downloadTimeline:@"gettipslikedbyuser" batch:++batchNo];
}
...
- (void)followTipTopicAtIndexPath:(NSIndexPath *)indexPath
{
    TipboxAppDelegate *appDelegate = (TipboxAppDelegate *)[[UIApplication sharedApplication] delegate];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (targetActionSheet.tag == 100) {        // Sharing options
        ...
        if (buttonIndex == 0) {                                                 // Follow topic
            [self followTipTopicAtIndexPath:targetActionSheet.indexPath];
